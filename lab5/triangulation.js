...
function computeTriangulation(points, boundaries) {
	...
	dcel = new DCEL(encTriangle[0], encTriangle[1], encTriangle[2]);
	...
	if (boundaries)
	{
		return vertices = pruneBoundaries(boundaries);
	}

	return vertices = parseTriangleVertices(dcel, true);
}

function pruneBoundaries(boundaries)
{
	dcel.pruneBoundaries(boundaries);
	return vertices = parseTriangleVertices(dcel, false);
}
...
function parseTriangleVertices(dcel, includeExternal)
{
	let vertices = [];

	for (let i = 1; i < dcel.faceTable.length; ++i)
	{
		if (!includeExternal && dcel.faceTable[i].external)
				continue;
